...
alter table public.ritua_products
  ...
  add column length_cm numeric check (length_cm > 0 and length_cm <= 10000),
  add column diameter numeric check (diameter > 0 and diameter <= 10000),
  add column diameter_unit text check (diameter_unit in ('cm','mm')),
  add column model text check (length(model) <= 120),
  add column compatible_with text check (length(compatible_with) <= 200),
  add column includes_lighter boolean,
  add constraint ritua_diameter_pair check ((diameter is null) = (diameter_unit is null)),
  add constraint ritua_piteira_details check (category='Piteira' or (length_cm is null and diameter is null and diameter_unit is null and model is null)),
  add constraint ritua_case_details check (category='Case' or (compatible_with is null and includes_lighter is null));
...
begin
  ...
  if payload->>'id' is not null then
    update public.ritua_products set name=trim(payload->>'name'), category=payload->>'category', description=coalesce(payload->>'description',''),
      ...
      model=v_model, compatible_with=v_compatible, includes_lighter=v_lighter
      where id = product_id returning * into p;
  else
    if quantity < 0 or quantity > 9999 then raise exception 'Quantidade inválida'; end if;
    insert into public.ritua_products(id,name,category,description,price_cents,published,is_unique,length_cm,diameter,diameter_unit,model,compatible_with,includes_lighter)
      values (product_id,trim(payload->>'name'),payload->>'category',coalesce(payload->>'description',''),(payload->>'priceCents')::integer,coalesce((payload->>'published')::boolean,false),v_unique,v_length,v_diameter,v_unit,v_model,v_compatible,v_lighter) returning * into p;
